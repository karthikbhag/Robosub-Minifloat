...
public class ConnectionManager {
    ...
    public void makePostConnection(String address) {
        outputDisplay.display("Sending POST request to: " + address);

        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(address))
                .timeout(Duration.ofSeconds(3))
                .header("Content-Type", "application/json")
                .POST(HttpRequest.BodyPublishers.noBody()) // POST with no body for now
                .build();
